...
//    );
    ...
    public function can($action, $controller = NULL)
    {
        ...
        //return App_FlagFlippers_Manager::isAllowed($user->id, $controller, $action);
	//New Enhancement Now using email instead of group
        if(isset($user->user_type) && ($user->user_type == HEAD_CORPORATE || $user->user_type == REGIONAL_CORPORATE || $user->user_type == LOCAL_CORPORATE)) {
            $username = $user->group->name;
        } else {
            $username = $user->email;
        }
        
        return App_FlagFlippers_Manager::isAllowed($username, $controller, $action);
        /**
         * Need to change
         * Right now All agent are administrator
         */
        //return App_FlagFlippers_Manager::isAllowed(AGENT_ADMIN, $controller, $action);
    ...
    public function link($link, $item, $echo = TRUE)
    {
        ...
        if (!isset($action['icon']) || (isset($action['icon']) && $action['icon'])) {
            $link = '<td>' . PHP_EOL .
                    ...
                    '<img class = "'.$action['img_class'].'" src="' . $this->_baseUrl . $action['img_url'] . 
            ...
            switch(TRUE){
                ...
                case isset($this->_columnDataTypes[$index]) && (!is_array($this->_columnDataTypes[$index]) && 
                 ...
                 case strtolower($index)  == 'card_number':
                            $result  = Util::maskCard($item,4,0);
                     break;
                 case strtolower($index)  == 'account_no':
                            $result  = Util::maskCard($item,4,0);
                     break;
                default:
                    $result = $item;

...
class BindAgentProductCommission extends App_Model
{
    /**
     ...
     * @access protected
     */
    protected $_name = DbTable::TABLE_BIND_AGENT_PRODUCT_COMMISSION;
     ...
     */
     public function getAgentProductLimitDetails($agentId, $productId)
     {
         ...
         $select = $this->_db->select()
                ->from(DbTable::TABLE_BIND_AGENT_PRODUCT_COMMISSION." as a")
   ...
   /*
    ...
    public function getAll($page = 1){
     ...
     public function getActiveAgentproductlimit($id){
         ...
         $select = $this->_db->select()    
                ->from(DbTable::TABLE_BIND_AGENT_PRODUCT_COMMISSION.' as a',array('a.*',"DATE_FORMAT(a.date_end, '%d-%m-%Y') as date_end_formatted",
    ...
    public function deleteBindAgentProductCommission($id, $prevId)
     { 
        if(isset($prevId)) 
        {
             $dataPrev = array(
                'date_end' => '0000-00-00' );
             $upd = $this->_db->update(DbTable::TABLE_BIND_AGENT_PRODUCT_COMMISSION, $dataPrev, "id = $prevId");
           if($upd)
           {
                $data = array('status' => 'inactive', 'date_end' => new Zend_Db_Expr('NOW()'));
                $this->_db->update(DbTable::TABLE_BIND_AGENT_PRODUCT_COMMISSION,$data, "id = '$id'");
                return true;
           }
        }
     ...
     public function chkDuplicateAgentProduct($agentId, $productId)
    {
        $select = $this->_db->select()
                ->from(DbTable::TABLE_BIND_AGENT_PRODUCT_COMMISSION)
     ...
     public function agentProduct(array $data){
        ...
        $res = $this->_db->insert(DbTable::TABLE_BIND_AGENT_PRODUCT_COMMISSION,$data);
        return $res;
     ...
     public function getAgentBinding($agentId, $date )
    {
        ...
        $this->_enableDbSlave();
         $select = $this->_db->select()
                 ->from(DbTable::TABLE_BIND_AGENT_PRODUCT_COMMISSION.' as b',array('b.agent_id','b.product_id', 'b.plan_commission_id' ))
...
//        echo $select;   
     ...
     public function getAgentBindingDur($agentId, $fromDate, $toDate, $productExclude) {
	$select = $this->_db->select()
		->from(DbTable::TABLE_BIND_AGENT_PRODUCT_COMMISSION . ' as b', array('b.agent_id', 'b.product_id', 'b.plan_commission_id'))
		->joinleft(DbTable::TABLE_PRODUCTS . ' as p', 'b.product_id=p.id', array('p.id as product_id', 'p.name as product_name', 'p.ecs_product_code as product_code', 'program_type'))
		->joinleft(DbTable::TABLE_BANK . ' as bk', 'p.bank_id=bk.id', array('bk.name as bank_name', 'bk.id as bank_id', 'bk.unicode as bank_unicode'))
	...
	//$select->where("b.date_start <= '".$date."' AND (b.date_end >= '".$date."' OR b.date_end = '0000-00-00')");

	$select->where("(DATE(b.date_start) between '" . $fromDate . "' AND '" . $toDate . "') OR (DATE(b.date_start) <= '" . $fromDate . "' AND ( DATE(b.date_end) >= '" . $fromDate . "' OR DATE(b.date_end) = '0000-00-00'))");

	if ($productExclude != '') {
	    $select->where("p.const NOT IN ('" . $productExclude . "')");
	}
	$commArr = $this->_db->fetchAll($select);
	return $commArr;
    }
    ...
    {
         $select = $this->_db->select()
                 ->from(DbTable::TABLE_BIND_AGENT_PRODUCT_COMMISSION.' as b',array('b.agent_id','b.product_id', 'b.plan_fee_id' ))
    ...
    {        
        ...
        if(strlen($mobile) <> Mobile::$length || !is_numeric($mobile)) {
            ...
            //$select = $this->select()
            $select = $this->_db->select();
            $select->from(DbTable::TABLE_BIND_AGENT_PRODUCT_COMMISSION.' as bapc',array('bapc.id'));
            ///$select->joinLeft("t_bind_agent_product_commission as bapc", "ch.id = bapc.agent_id AND bapc.product_id=$productId AND ".$cuDate." >= bapc.date_start AND (".$cuDate." <= bapc.date_end OR bapc.date_end = '0000-00-00')", array('bapc.id as bapc_id'));                    
            $where = "bapc.agent_id = '".$agentId."' AND bapc.product_id = '".$productId."'"; 
     ...
     */
     public function getAgentProductForCurrentDate($agentId)
     {
         $curdate = new Zend_Db_Expr('NOW()');
         $select = $this->_db->select()
                ->from(DbTable::TABLE_BIND_AGENT_PRODUCT_COMMISSION." as a", array('product_id'))
                ->where("a.agent_id = ?", $agentId)
                ->where("$curdate >= a.date_start AND ($curdate <= a.date_end OR a.date_end = '0000-00-00' OR a.date_end is NULL)");
         //echo $select->__toString();exit;
          $resp = $this->_db->fetchRow($select);
     ...
     * getAgentProductForCurrentDate() will find n return agent product allowed with him for current date
     */
     public function getAgentProductAndBank($agentId)
     {
         $curdate = date('Y-m-d');
         
         $select = $this->_db->select();
         $select->from(DbTable::TABLE_BIND_AGENT_PRODUCT_COMMISSION." as bind", array('bind.product_id'));
         ...
         $select->where("bind.agent_id = ?", $agentId);
     ...
     * @return type
     */  
    public function getAgentProduct($agentId, $productId)
    {
        $curdate = new Zend_Db_Expr('NOW()');
        $select = $this->_db->select()
                ->from(DbTable::TABLE_BIND_AGENT_PRODUCT_COMMISSION)
                ->where("agent_id=".$agentId)
                ->where("product_id = '".$productId."'")
                ->where("$curdate >= date_start AND ($curdate <= date_end OR date_end = '0000-00-00' OR date_end is NULL)");
                //->where("'".$row['date_start']."' BETWEEN date_start AND date_end AND date_end != '0000-00-00'");
        //echo $select->__toString();exit;
        return $this->_db->fetchRow($select);
    }
     ...
     public function getProductAgents($param)
     {
         $productList = $param['product_id_list'];
         $fromDate = explode(" ",$param['from']);
         $select = $this->_db->select()
                 ->from(DbTable::TABLE_BIND_AGENT_PRODUCT_COMMISSION ." as bind")
                 ->where("$fromDate[0] <= bind.date_end OR bind.date_end = '0000-00-00' OR bind.date_end is NULL")
                 ->where("bind.product_id IN($productList)");
         //echo $select->__toString();exit;
          ...
          if(empty($resp)){
              return '';
               ...
               foreach($resp as $agent){
          $agent_ids .= $agent['agent_id'].",";
        }
        // remove last ,
        return  substr($agent_ids,0,-1);
          }
              
     }
     
     public function getAgentBindingCommition($param )
    {
         $agentId = isset($param['agent_id'])?$param['agent_id']:0;
         $productId = isset($param['product_id'])?$param['product_id']:0;
         $typeCode = isset($param['typecode'])?$param['typecode']:'';
         $curdate = new Zend_Db_Expr('NOW()');
        //Enable DB Slave
        $this->_enableDbSlave();
         $select = $this->_db->select()
            ...
            if($agentId > 0)
                ...
                if($productId > 0)
                {
                    $select->where("b.product_id =?", $productId);
                }
                if($typeCode !='')
                {
                    $select->where("ci.typecode =?", $typeCode);
                }
                $select->where("$curdate >= date_start AND ($curdate <= date_end OR date_end = '0000-00-00' OR date_end is NULL)");
                
        $commArr =  $this->_db->fetchRow($select);
        //Disable DB Slave
        $this->_disableDbSlave();
        return $commArr;
     }

 ...
 * @copyright transerv
 */
class App_ApiServer_Exchange_Package_Ratnakar_GPR extends App_ApiServer_Exchange_Package_Ratnakar
{
    const TP_ID = TP_RAT_GPR_ID;
    private $_soapServer;
    ...
    public function __construct($server) {
        ...
        $this->setAgentConstant(RBL_GPR_AGENT_ID);
    }
...
public function CardRegistrationRequest () {//Do not add comments for method summary
        try {
            ...
            if( !isset($resp->ProductCode) || empty($resp->ProductCode) || !$this->validateProductCode($resp->ProductCode) || !$this->validateProductCodeByConst($resp->ProductCode,PRODUCT_CONST_RAT_GPR)) {
...
//                 return self::Exception($this->getMessage('invaild_mobile'), App_ApiServer_Exchange::$INVALID_RESPONSE);
    ...
    public function CardTransactionRequest ($obj) {//Do not add comments for method summary
        try {
            ...
            } elseif($this->isWalletAllowed(BANK_RATNAKAR_GENERIC_GPR, $resp->WalletCode)) {
                 return self::Exception("Invalid Wallet Code", App_ApiServer_Exchange::$INVALID_RESPONSE);                
            }
            ...
            try {
                ...
                if(strtolower($resp->TxnIdentifierType) == strtolower(self::TXN_IDENTIFIER_TYPE_CRN)) {
                    $param = array(

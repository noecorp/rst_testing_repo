...
class Approvecorporate extends App_Model
{
    /**
    ...
    public function getCorporateCode($id){
     ...
     public function getDetails ($param, $page = 1, $paginate = NULL, $force = FALSE){
       ...
       $details = $this->select()
                 ->from(DbTable::TABLE_CORPORATE_USER." as c",array('id','c.email','c.mobile','corporate_code','c.first_name', 'c.last_name',"DATE_FORMAT(c.date_created,'%d-%m-%Y') as reg_datetime",'c.date_created as datetime'))
                ...
                ->where ("c.status ='".STATUS_UNBLOCKED."'");
        ...
        if(!empty($columnName) && !empty($keyword))
        {
            ...
            $details->where($whereString);
        }
        
        $details->order('datetime DESC');
    ...
    public function getrejectedDetails ($page = 1, $paginate = NULL, $force = FALSE){
       ...
       $details = $this->select()
                 ->from(DbTable::TABLE_CORPORATE_USER,array('status','id','email','mobile','corporate_code','concat(first_name," ",last_name) as name',"DATE_FORMAT(date_created,'%d-%m-%Y') as reg_datetime"))
    ...
    public function getcorporateproductDetails ($page = 1, $id){
        $paginate = NULL;
        $force = FALSE;
       $details = $this->getcorporateproductSql($id);
     ...
     //exit;
      ...
      private function getcorporateproductSql($id = 0){
         $details = $this->select()
    ...
    public function getcorporateDetails($id){
        
       $details = $this->select()
                ->from(DbTable::TABLE_CORPORATE_USER." as a", array('a.email','a.first_name','a.last_name'))
                ->where ("a.id = $id");
    ...
    public function deleteById($agentId){
     ...
     public function approveById($agentId,$data){
        ...
        $agent_code = $this->getCorporateCode($agentId);
        ...
        //Alert method for sending password in email and SMS
         ...
         $corporategroupModel = new CorporateUserGroup();
         if(isset($param['user_type']) && $param['user_type']==SUPER_CORPORATE_DB_VALUE) {
            $corporategroupModel->saveForUser($param['id'],HEAD_CORPORATE_GROUP);
         }elseif(isset($param['user_type']) && $param['user_type']==DISTRIBUTOR_CORPORATE_DB_VALUE) {
            $corporategroupModel->saveForUser($param['id'],REGIONAL_CORPORATE_GROUP);
         }else{
            $corporategroupModel->saveForUser($param['id'],LOCAL_CORPORATE_GROUP);
         }
         
         
         $detailsArr = array('id' =>$param['id'],'first_name'=>$param['first_name'],'last_name'=>$param['last_name'],'name'=>$param['name'],
             'email'=>$param['email'],'auth_email'=>$param['auth_email'],'password'=>$password,'corporate_code'=>$agent_code);
     ...
     public function sendVerificationCode($id,$verification_code,$detailsArr){
        ...
        $check = $this->_db->select()
            ...
            $activationArr = array ('corporate_id'=>$id,
     ...
     public function getBank()
    {
        ...
        $select->where("unicode IN('300', '400')");
        $select->setIntegrityCheck(false);
    ...
    public function findCorporate($id){
        if (!is_numeric($id)) {
            return array();
        }
        
        $select = $this->_db->Select()
                   ->from(DbTable::TABLE_CORPORATE_USER." as a",array('a.*','concat(a.first_name," ",a.last_name) as name'))
                    ->where("a.id=".$id);

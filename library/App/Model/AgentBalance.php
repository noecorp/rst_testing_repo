...
class AgentBalance extends BaseUser
{
    /**
...
//                        $agentArr['status'] = STATUS_DEBITED;
                      ...
                      //  $agentDebitUnsettledReversal = $paytronicLoadModel->getAgentDebitUnsettledReversals($agentArr);
                         ...
                         $paytronicReversal['total_agent_load_amount'] = (isset($paytronicReversal['total_agent_load_amount']))? $paytronicReversal['total_agent_load_amount']: 0;
                          ...
                          $loadFee = (isset($paytronicCorpLoad['agent_total_load_fee']))? $paytronicCorpLoad['agent_total_load_fee']: 0;
                          $loadSTax = (isset($paytronicCorpLoad['agent_total_load_stax']))? $paytronicCorpLoad['agent_total_load_stax']: 0;
                        //  $agentDebitUnsettledReversal['total_agent_load_amount'] = (isset($agentDebitUnsettledReversal['total_agent_load_amount']))? $agentDebitUnsettledReversal['total_agent_load_amount']: 0;
                         
                         $totalComm = $txnAgentObj->getAgentTotalForComm($agentArr);
                         $totalCommReversal = $txnAgentObj->getAgentTotalForCommReversal($agentArr);
//			error_log('totalComm:' .$totalComm);
//			error_log('totalCommReversal:' .$totalCommReversal);


                         $addOnOpeningBal = $agentFunding['total_agent_funding_amount'] 
                                            + $agentRefundRequest['agent_total_remittance_refund'] 
                                            + $agentRefundRequest['agent_total_reversal_refund_stax'] 
                                            + $agentRefundRequest['agent_total_reversal_refund_fee']
                                            + $agentFundTrfrCr['total_agent_fundtrfr_amount']
                         ...
                         $subtractOnOpeningBal = $agentCardLoads['total_agent_load_reload'] 
                                            ...
                                            + $agentDebitReversal['total_agent_load_amount']
                                            + $loadFee
                                            + $loadSTax
                                            + $totalCommReversal;
...
//            echo $select; //exit;    
     ...
     * @return boolean
     */
    public function getAgentBalanceInfo($agentId){
        $select = $this->_select()
                ->where('agent_id = ?', $agentId);        
        echo $select;
        $rs = $this->fetchRow($select);  
     ...
     public function getAgentListClosingBalance($param) {
         $agentIds = isset($param['agent_id_list'])?$param['agent_id_list']:'';
         $date = isset($param['date'])?$param['date']:'';
         $select  = $this->_db->select()        
            ->from(DbTable::TABLE_AGENT_CLOSING_BALANCE, array('sum(closing_balance) as closing_balance'))
    ...
    public function getAgentBalanceLock($agentId){
        $select = $this->select()
                ->forUpdate()
                ->from($this->_name, array('amount'))
                ->where('agent_id = ?', $agentId);     

...
/*
 * Validator class for Ratnakar remittance
 * 
 */
class Remit_Ratnakar_Validator extends Validator_LimitValidator
{
  ...
  /**
    ...
    public function chkAllowRemitterRegn($params)
    {
        if($params['agent_id'] == '' || $params['amount'] == '' )
            throw new App_Exception ('Insufficient Data for validating Ratnakar Remitter Regn.');
    ...
    public function chkRemitterLimits($params)
    {
        ...
        if($prodLimitDetails)
        {
            ...
            $flgRemitterTxnRange = $this->getMinMaxLimitFlag($txnAmount, RATNAKAR_REMITTANCE_MIN_AMOUNT_PER_TXN, $prodLimitDetails['customer_limit_out_max_txn'], "Remitter");
...
public function validateRemitterMonthlyLimit($params)
    {
    	$limitValidatorResponse = array();
    	$agentProductModel = new BindAgentProductCommission();
    	$prodLimitDetails = $agentProductModel->getAgentProductLimitDetails($params['agent_id'], $params['product_id']);
    	...
    	$startDate = $curYear.'-'.$curMonth.'-01';
    	$endDate = $curYear.'-'.$curMonth.'-'.$curMonthDays;
    	$row = $txnRemitterModel->getTxnRemitterProductDuration($params['remitter_id'], $params['product_id'], $startDate, $endDate);
    	$limitValidatorResponse['limit']=$prodLimitDetails['customer_limit_out_max_monthly'];

    error_log('Monthly limit :');	
	error_log($limitValidatorResponse['limit']);
    error_log('Total till now :');
error_log($row['total']);
    	if($row) {
    		$total = ($row['total'] > 0) ? $row['total'] : 0;
    		try{
    			$result = $this->getRemitterLimitFlag($total, $params['amount'], $limitValidatorResponse['limit'], "Monthly", "Remitter for this Product");
    			$limitValidatorResponse['success'] = $result;
    		...
    		$limitValidatorResponse['success'] = true;
    		return $limitValidatorResponse;
    	}
    }

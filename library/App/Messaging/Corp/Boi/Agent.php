 ...
 * @author Vikram
 */
class Agent extends \App\Messaging\Corp\Boi {
      ...
      public function cardLoadAuth($userData,$resend = FALSE)
    {
        ...
        try {
             return TRUE;//Stopping BOI SMS till further notice     
      ...
      public function cardLoadSuccess($userData,$resend = FALSE)
    {
      ...
      public function verificationCode($userData) {
        ...
        try {
            return TRUE;
            $session = new \Zend_Session_Namespace('App.Agent.Controller');  
            if(isset($session->ver_code) && $session->ver_code != ''){
                $userData['v_code'] = $session->ver_code;
            }
            else{
            $userData['v_code'] = $this->generateRandom6DigitCode();
            }
            
            $session->ver_code = $userData['v_code'];

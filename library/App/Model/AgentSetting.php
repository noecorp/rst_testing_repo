...
class AgentSetting extends App_Model
{
    /**
    ...
    public function getAgentSettingValue($type){
     ...
     public function getAllAgentMinMaxSetting($param){        
           
            $select = $this->getAgentBalanceSettingSql($param);
            //echo $select->__toString();exit;        
        
        return  $this->fetchAll($select);       
    }
    
    private function getAgentBalanceSettingSql($param){
        $type = isset($param['type'])?$param['type']:'';
        
        $select =   $this->select();
        $select->setIntegrityCheck(false);
        $select->from(DbTable::TABLE_AGENT_BALANCE_SETTINGS.' as s',array("s.id, DATE_FORMAT(s.date_created, '%d-%m-%Y %h:%i:%s') as date_created", "s.description","s.agent_id", "s.value", "s.ip", "s.type"));
        $select->joinLeft(DbTable::TABLE_OPERATION_USERS.' as ou',"s.by_ops_id = ou.id ",array('ou.username as ops_name'));
        $select->where('s.status= ?', STATUS_ACTIVE);

...
class Remit_Kotakideacts_ReportsController extends App_Agent_Controller
{
    /**
    ...
    {   $user = Zend_Auth::getInstance()->getIdentity();
        ...
        $form = new Remit_Kotak_RemittanceReportForm(array('action' => $this->formatURL('/remit_kotakideacts_reports/remittancereport'),
                                                              'method' => 'POST',
                                             ));  
     ...
     */
     ...
     public function exportremittancereportAction(){
         $user = Zend_Auth::getInstance()->getIdentity();       
        // Get our form and validate it
        $form = new Remit_Kotak_RemittanceReportForm(array('action' => $this->formatURL('/remit_kotakideacts_reports/exportremittancereport'),
                                                              'method' => 'POST',
                                             )); 
        //$formData  = $this->_request->getPost();//$form->getValues();//
        $qurStr['dur'] = $this->_getParam('dur');
              ...
              if($form->isValid($qurStr)){ 
                 ...
                 try{
                        $resp = $objCSV->export($exportData, $columns, 'agent_remittance');exit;
                                            ...
                                            $this->_redirect($this->formatURL('/remit_kotakideacts_reports/exportremittancereport?dur='.$qurStr['dur'].'&sub=1&id='.$qurStr['id'].'&bank_unicode='.$qurStr['bank_unicode'])); 
                                        }
                    ...
                    $this->_redirect($this->formatURL('/remit_kotakideats_reports/exportremittancereport?dur='.$qurStr['dur'].'&sub=1&id='.$qurStr['id'].'&bank_unicode='.$qurStr['bank_unicode'])); 
                 }    
       }
         /*
    ...
    public function remittancecommissionAction(){
        $this->title = 'Remittance Commission Report';           
         // Get our form and validate it
        $form = new Remit_Boi_RemittanceCommForm(array('action' => $this->formatURL('/remit_kotakideacts_reports/remittancecommission'),
                                                    'method' => 'POST',
                                         )); 
              ...
              if($form->isValid($qurStr)){ 
                 ...
                 try{
                        $resp = $objCSV->export($exportData, $columns, 'remittance_commission_report');exit;
                                            ...
                                            App_Logger::log($e->getMessage() , Zend_Log::ERR);
                                            $this->_helper->FlashMessenger( array('msg-error' => $e->getMessage(),) );
                                             $this->_redirect($this->formatURL('/remit_kotakideacts_reports/remittancecommission?duration='.$qurStr['duration'].'&sub=1&bank_unicode='.$qurStr['bank_unicode'])); 
                                        }

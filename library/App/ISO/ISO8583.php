...
/**
 * ISO8583
 * Class to manage iso8583 type data
 * 
 * @copyright(c) Transerv
 * @author Vikram Singh <vikram@transerv.co.in>
 */   
class App_ISO_ISO8583 {
    ...
    private $_msg	= '';//4 Byte ASCII Length
    private $_isoliterals= '';//ISO Literals //3 CHAR Must be ISO
    private $_header	= '';//9 Byte ASCII Header
    ...
    public function addMessage($msg = '0067') {
        if (strlen($msg)==4 ) {
            $this->_msg	= $msg;
        }        
    }
    ...
    public function addISOLiterals($ISO = 'ISO') {
        $this->_isoliterals = $ISO;
    }
    
    public function getISOLiterals() {
        return $this->_isoliterals;
    }
    ...
    public function getISOwithHeaders() {
        //$this->_isoheader = $this->_msg. $this->_isoliterals. $this->_header. $this->_mti. $this->_bitmap. implode($this->_data);
        $length = strlen($this->_isoliterals. $this->_header. $this->_mti. $this->_bitmap. implode($this->_data));
        //print $length.'***';
        //print dechex($length)."***";
        if(strlen(dechex($length)) == 3 ) {
            $length = '0'.  strtoupper(dechex($length));
        } else {
            $length = '00'.  strtoupper(dechex($length));
        }
        //print $length.'**';
        //$length = '00'.  strtoupper(dechex($length));
        $this->_isoheader = $length. $this->_isoliterals. $this->_header. $this->_mti. $this->_bitmap. implode($this->_data);
        return $this->_isoheader;
    }
    
    
    //method: add ISO string
    public function addISOwithHeader($iso) {
        $this->_clear();
        if ($iso!='') {
            $iso2 = $this->_parseHeaders($iso);
            $this->addISO($iso2);         
        }
    }
    
    public function _parseHeaders($iso)
    {
        $this->addMessage(substr($iso, 0, 4));
        $this->addISOLiterals(substr($iso, 4, 3));
        $this->addAdditionalHeader(substr($iso, 7, 9));
        return substr($iso,16);
    }

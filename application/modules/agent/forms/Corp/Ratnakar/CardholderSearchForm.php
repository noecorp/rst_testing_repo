<?php
/**
 * Search Cardholder Form
 *
 * @category agent
 * @package agent_forms
 * @copyright Transerv
 */

class Corp_Ratnakar_CardholderSearchForm extends App_Agent_Form
{
    /**
     * This form does not have a cancel link
     * 
     * @var mixed
     * @access protected
     */
    protected $_cancelLink = false;
    
    /**
     * Overrides init() in Zend_Form
     * 
     * @access public
     * @return void
     */
    public function init() {
        // init the parent
        parent::init();
        
        // set the form's method
        //$this->setMethod('get');
        
        $medi_assist_id = new Zend_Form_Element_Text('medi_assist_id');
        $medi_assist_id->setOptions(
            array(
                'label'      => 'Medi Assist Id',
                'required'   => false,
                'filters'    => array(
                                    'StringTrim',
                                    'StripTags',
                                ),
                'validators' => array('Digits',array('StringLength', false, array(2, 10)),),
                'maxlength' => '10',
            )
        );
        $this->addElement($medi_assist_id);
        
        $employer_name = new Zend_Form_Element_Text('employer_name');
        $employer_name->setOptions(
            array(
                'label'      => 'Employer Name',
                'filters'    => array('StringTrim'),
                'validators' => array('NotEmpty', array('StringLength', false, array(3, 100)),),
                'required'   => false,
                'maxlength'  => '100',
            )
        );
        $this->addElement($employer_name);
        
        $card_number = new Zend_Form_Element_Text('card_number');
        $card_number->setOptions(
            array(
                'filters'    => array('StringTrim'),
                'validators' => array('NotEmpty','Digits', array('StringLength', false, array(10, 10))),
                'required'   => false,
                'label'      => 'Card Number',
                'maxlength'  => '10',
            )
        );
        $this->addElement($card_number);
        
        
        $employee_id = $this->addElement('text', 'employee_id', array(
            'filters'    => array('StringTrim'),            
            'validators' => array('NotEmpty','Alnum', array('StringLength', false, array(2, 10))),
            'required'   => false,
            'label'      => 'Employee ID',
            'maxlength'  => '10',
        ));
        
        
        $mobile = new Zend_Form_Element_Text('mobile');
        $mobile->setOptions(
            array(
                'filters'    => array('StringTrim'),
                'validators' => array('NotEmpty','Digits', array('StringLength', false, array(10, 10)),),
                'required'   => false,
                'label'      => 'Mobile Number',
                'maxlength'  => '10',
            )
        );
        $this->addElement($mobile);
        
        $email = new Zend_Form_Element_Text('email');
        $email->setOptions(
            array(
                'filters'    => array('StringTrim'),
                'validators' => array('EmailAddress', array('StringLength', false, array(5, 50)),),
                'required'   => false,
                'label'      => 'Email',
                'maxlength'  => '50',
            )
        );
        $this->addElement($email);
        
        $aadhaar_no = new Zend_Form_Element_Text('aadhaar_no');
        $aadhaar_no->setOptions(
            array(
                    'filters'    => array('StringTrim'),
                    'validators' => array('Digits', array('StringLength', false, array(12, 12)),),
                    'required'   => false,
                    'label'      => 'Aadhaar No. (UID)',
                    'maxlength'  => '12',
                 )
        );
        $this->addElement($aadhaar_no);
        
        
        $pan = new Zend_Form_Element_Text('pan');
        $pan->setOptions(
            array(
                'filters'    => array('StringTrim'),
                'validators' => array('Alnum', array('StringLength', false, array(10, 10)),),
                'required'   => false,
                'label'      => 'PAN',
                'maxlength'  => '10',
            )
        );
        $this->addElement($pan);
        
        
        $submit = new Zend_Form_Element_Submit('submit_form');
        $submit->setOptions(
            array(
                'label'      => 'Search Cardholder',
                'required'   => FALSE,
                'ignore'   => true,
                'title'       => 'Search Cardholder',
                'class'     => 'tangerine',
            )
        );
        $this->addElement($submit);
        
        
        
        
        $this->setElementDecorators(array(
                    'viewHelper',
                    'Errors',
                    array(array('data'=>'HtmlTag'),array('tag'=>'dd','class'=>'form-field-column edit')),
                    array('Label',array('tag'=>'dt','class'=>'form-name-column')),
                    
        ));
                // We want to display a 'failed authentication' message if necessary;
        // we'll do that with the form 'description', so we need to add that
        // decorator.
        $this->setDecorators(array(
            'FormElements',
            //array('HtmlTag', array('tag' => 'div', 'class' => 'innerbox')),
            array(array('Value'=>'HtmlTag'), array('tag'=>'dl','class'=>'innerbox form')),
            array('Description', array('placement' => 'prepend')),
            'Form'
        ));
    }
}
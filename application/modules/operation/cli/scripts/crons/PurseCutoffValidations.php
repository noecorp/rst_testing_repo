...
$crons = new Crons();
$cronInfo = $crons->getCronInfo(array('cron_id'=>CRON_PURSE_CUT_OFF_VALIDATION));
...
if($activeStatus==STATUS_ACTIVE){ // if cron active then only cron should execute
   ...
   if($runStatus==STATUS_COMPLETED) { //if cron in execution status as 'complete' then only will run
    try{
        ...
        $crons->updateCron(array('status_cron'=>STATUS_STARTED, 'id'=>CRON_PURSE_CUT_OFF_VALIDATION)); // updating cron status in t_cron table
        //echo $cronLogId.'---------'; die;
        ...
        $cutoff = $clearLoadResp['cutoff'] + $clearLoadboiResp['cutoff'];
        $notcutoff = $clearLoadResp['not_cutoff'] + $clearLoadboiResp['not_cutoff'];
        ...
        if($countExcep>0){
            foreach($exceptions as $key=>$val){
                    $cronData = array(
                                       'cron_id'=>CRON_PURSE_CUT_OFF_VALIDATION,
                                       'message'=>$val,
                                       'date_start'=>$now,

...
$crons = new Crons();
$cronInfo = $crons->getCronInfo(array('cron_id'=>CRON_NEFT_RESPONSE_SEND_SMS_ID));
...
if($activeStatus==STATUS_ACTIVE){ // if cron active then only cron should execute
    ...
    $cronLogId = $crons->addCronLog(array('cron_id'=>CRON_NEFT_RESPONSE_SEND_SMS_ID));
    //echo $cronLogId.'---------'; die;
...
if($runStatus==STATUS_COMPLETED) { //if cron in execution status as 'complete' then only will run
    ...
    try{
        ...
        $crons->updateCron(array('status_cron'=>STATUS_STARTED, 'id'=>CRON_NEFT_RESPONSE_SEND_SMS_ID)); // updating cron status in t_cron table
        ...
        $param = array('cron_id'=>CRON_NEFT_RESPONSE_SEND_SMS_ID, 'message'=>$msg, 'id'=>$cronLogId);
        $crons->updateCron(array('status_cron'=>STATUS_COMPLETED, 'id'=>CRON_NEFT_RESPONSE_SEND_SMS_ID)); // updating cron status in t_cron table
    } catch(Exception $e){
        $msg = $e->getMessage();
        $param = array('cron_id'=>CRON_NEFT_RESPONSE_SEND_SMS_ID, 'message'=>$msg, 'id'=>$cronLogId);
        $crons->updateCron(array('status_cron'=>STATUS_STOPPED, 'id'=>CRON_NEFT_RESPONSE_SEND_SMS_ID)); // updating cron status in t_cron table
      }
} else {
    $param = array('cron_id'=>CRON_NEFT_RESPONSE_SEND_SMS_ID, 'message'=>'That cron already '.$runStatus.' and does not has complete status', 'id'=>$cronLogId);
}

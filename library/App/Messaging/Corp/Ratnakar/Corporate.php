 ...
 * @author Vijay
 */
class Corporate extends \App\Messaging\Corp\Ratnakar {
    ...
    */ 
    public function cardholderAuth($userData ,$resend = FALSE)
    {
        ...
        try {
                ...
                $productId = isset($userData['product_id'])?$userData['product_id']:'';
                if ($resend){
                ...
                if($mobNo!='' && $userData['mobile_number_old']!=$mobNo) {
                 try {                
                        $mobCheck = $mobObj->checkRatCardholderMobileDuplicate($mobNo,$productId);                
                   } catch (\Exception $e ) {
                       \App_Logger::log($e->getMessage(), \Zend_Log::ERR);
                       $msg = $e->getMessage();
                       $isError = true;
                ...
                } else if($mobNo==''){ throw new \Exception ("No mobile number provided"); $isError = true;}                 

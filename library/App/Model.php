...
abstract class App_Model extends Zend_Db_Table_Abstract
{
     ...
     * @return mixed
     */
    protected function _paginate($select, $page, $paginate, $itemsPerPage = '' ){
        if (NULL === $paginate) {
    ...
    {
     return Util::restoreIpAddressFromat($ip);
    ...
    public function encode256($plainText,$key) {
        if(empty($key)) {
            $key = $this->getKey();
        }
     ...
     * @return void
     */
    protected function _setupReadDatabaseAdapter($enable){
        if(strtolower($enable) == 'y') {
            $this->_db = Zend_Registry::get('dbreadAdapter');
        } else {
            $this->_setupDatabaseAdapter();  
        }
    }
    
    public function _enableDbSlave() {
        $this->_setupReadDatabaseAdapter('y');
    }
    
    public function _disableDbSlave() {
        $this->_setupReadDatabaseAdapter('n');
    }
}
...
class Validator_Ratnakar_Customer extends Validator_Ratnakar {
    ...
    public static function narrationValidation($narration) {
        $cust = new self();
        if ($narration != '') {
             if( (strlen($narration) > 40)  || (!$cust->chkallowChar($narration)) ) {
    ...
    public static function walletCodeValidation($walletcode) {
        if($walletcode != '') {
            if(strlen($walletcode) > 6) {
                throw new Exception(ErrorCodes::ERROR_EDIGITAL_INVALID_WALLET_MSG, ErrorCodes::ERROR_EDIGITAL_INVALID_WALLET_CODE);
            }
       }
    }

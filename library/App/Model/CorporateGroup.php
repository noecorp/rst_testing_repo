...
class CorporateGroup extends App_Model
{
    /**
     ...
     * @access protected
     */
    protected $_name = DbTable::TABLE_CORPORATE_GROUP;
    ...
    public function existsGroupName($name) {
        $details = $this->select()
                ->from(DbTable::TABLE_CORPORATE_GROUP." as g", array('id'))
                ->where("g.name='$name'");
    ...
    public function findAll ($page = 1,$paginate = NULL, $force = FALSE){
       ...
       $details = $this->select()
               ->from(DbTable::TABLE_CORPORATE_GROUP." as g",array('id','name'))
                ->setIntegrityCheck(false) 
                ->joinleft(DbTable::TABLE_CORPORATE_USER_GROUP." as og","g.id=og.group_id",array('count(`og`.`group_id`) as count'))
                ->group('g.id')
                ->order('g.name asc');
            //echo $details->__toString();   exit; 

 ...
 * @copyright transerv
 */
class App_ApiServer_Exchange_Services_Copass extends App_ApiServer_Exchange_CommonServices
{
    private $_soapServer;
    
    const TP_ID = TP_COPASS_ID;
    ...
    public function __construct($server) 
    {
        ...
        $this->setProductConst($productConst = PRODUCT_CONST_RAT_COP);
    }
    ...
    public function CardRegistrationRequest () {//Do not add comments for method summary
        try {
            ...
            if( !isset($resp->ProductCode) || empty($resp->ProductCode) || !$this->validateProductCode($resp->ProductCode) || !$this->validateProductCodeByConst($resp->ProductCode,PRODUCT_CONST_RAT_COP)) {
...
//                    return $responseObj;
                ...
                $params['by_api_user_id'] = COPASS_AGENT_ID;
...
//                return $responseObj;                
               ...
               $otpResponse = $refObject->verifyCustomerLoadOTP($otpVeriParam);
                ...
                $param = array(
                    ...
                    'by_api_user_id'        => (string) COPASS_AGENT_ID,
		    'channel'               => CHANNEL_API,
                );
                $response = $baseTxn->doCardload($param);
                $errorMsg = $baseTxn->getError();
                //$responseObj = new stdClass();              
                if($response['status'] == STATUS_LOADED || $response['status'] == STATUS_DEBITED) {
                    $responseObj = new stdClass();
                    $responseObj->SessionID      = (string) $resp->SessionID;
                    $responseObj->TxnNo          = (string) $resp->TxnNo;
    ...
    private function getLoadFailedMsg($txnType) {
        if(strtolower($txnType) == strtolower(TXN_MODE_CR)) {
            return self::LOAD_FAILED_RESPONSE_MSG;
        } else {
            return self::DEBIT_FAILED_RESPONSE_MSG;
        }
    }

...
class Remit_Remittancestatuslog extends Remit_Boi
{
    /**
     ...
     * @access protected
     */
    protected $_name = DbTable::TABLE_REMITTANCE_STATUS_LOG;
    ...
    public function addStatus($param)
    {
       ...
       try {       
          return $this->_db->insert(DbTable::TABLE_REMITTANCE_STATUS_LOG,$param); 
       }
       catch(Exception $e ) {
     ...
     */
     public function getRemittanceRefundYetToClaim($param){
        ...
        $select->from(DbTable::TABLE_REMITTANCE_STATUS_LOG." as rsl", array('rsl.date_created'));              
        $select->joinInner(DbTable::TABLE_REMITTANCE_REQUEST." as rr", "rsl.remittance_request_id =rr.id", array('amount', 'neft_remarks as remarks','neft_remarks', 'txn_code', 'service_tax', 'fee', 'status', 'batch_name', 'batch_date', 'neft_processed', 'neft_processed_date', 'status_sms', 'date_updated as txn_date_updated'));              
        $select->joinLeft(DbTable::TABLE_REMITTERS." as r", "rr.remitter_id =r.id", array('r.name as remitter_name', 'mobile as remitter_mobile', 'email as remitter_email'));
        $select->joinLeft(DbTable::TABLE_BENEFICIARIES." as b", "rr.beneficiary_id = b.id", array('b.name as beneficiary_name', $bankAccountNumber, 'b.by_agent_id'));
       ...
       // echo $select->__toString();//exit;
        ...
        if($rsCount > 0)
        {
            foreach($rows as $val)
            {
                ...
                $usertype = $agentUser->getAgentDetailsById($val['agent_id']);
                $agentType = $agentUser->getAgentCodeName($usertype['user_type'], $val['agent_id']);
                $getBankame = $agentUser->getAgentBankName($val['agent_id']);

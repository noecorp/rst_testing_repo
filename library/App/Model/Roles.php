...
class Roles extends App_Model
{
     ...
     * @access protected
     */
    protected $_name = DbTable::TABLE_ROLES;
    ...
    public function findAll ($page = 1,$paginate = NULL, $force = FALSE){
       ...
       $details = $this->select()
               ->from(DbTable::TABLE_ROLES." as r",array('id','name'))
                ->setIntegrityCheck(false) 
                ->joinleft(DbTable::TABLE_USER_ROLES." as up","r.id=up.roles_id",array('count(`up`.`roles_id`) as count','up.module'))
                ->group('r.id')
                ->order('r.id asc');
            //echo $details->__toString();   exit; 
